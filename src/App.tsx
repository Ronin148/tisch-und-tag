...
import { addDays, blankRecipe, coveredByPantry, ingredientText, isChecked, localDate, monday, shoppingList, shoppingSignature, uid, weekDates, weekLabel, validateBackup } from './domain';
...
export default function App() {
  ...
  const items = useMemo(() => data ? shoppingList(data, week, usePantry) : [], [data, week, usePantry]);
  ...
  async function copyShopping() {
    if (!data) return; const text = `Einkaufsliste · ${weekLabel(week)}\n\n${items.map(i => `${isChecked(data, week, i) ? '✓' : '☐'} ${coveredByPantry(i)?i.name+' (im Vorrat)':ingredientText(i)}`).join('\n')}`;
  ...
  return <div className="app-shell">
    ...
    <main id="main" className="main-content">
      <header className="topline"><span>DEIN PERSÖNLICHES KOCHBUCH</span><button onClick={() => {setKitchenTab('family');showView('kitchen');}}><span className="status-dot" /><Smartphone size={14} />{family.connected?(family.conflict?'Abgleich nötig':family.status):'Familie verbinden'}</button></header>
      ...
      {profileDirty&&(view!=='kitchen'||kitchenTab!=='profiles')&&<div className="notice draft-notice"><span>Deine Profiländerungen sind noch als Entwurf offen.</span><button className="text-btn" onClick={()=>{setKitchenTab('profiles');showView('kitchen');}}>Weiter bearbeiten</button></div>}
      <div hidden={view!=='kitchen'}><Kitchen data={data} family={family} tab={kitchenTab} setTab={setKitchenTab} onProfileDirty={setProfileDirty} onChange={commit} onAdd={saveRecipe} onOpen={setDetailId} /></div>
      ...
      {view === 'recipes' && <>
        {!data.recipes.length&&<section className="empty-collection-intro"><img src={`${import.meta.env.BASE_URL}images/bowl.webp`} alt="Bunte Gemüseschale"/><p>Deine Rezepte aus Fotos, Links und Texten – an einem Ort.</p></section>}
        <div className="section-heading"><div className="section-title"><h1>Meine Rezepte</h1><span className="count-pill">{data.recipes.length}</span></div><div className="quick-import"><button onClick={() => setEditor({ tab: 'photo' })}><Camera size={17} />Foto scannen</button><span /><button onClick={() => setEditor({ tab: 'link' })}><LinkIcon size={17} />Link importieren</button><span /><button onClick={() => setEditor({ tab: 'text' })}><ClipboardPaste size={17} />Text einfügen</button></div></div>
        <div className="collection-tools"><div className="search-field"><Search size={19} /><input aria-label="Rezepte suchen" placeholder="Nach Rezept oder Zutat suchen …" value={search} onChange={e => setSearch(e.target.value)} />{search && <button className="icon-btn" aria-label="Suche löschen" onClick={() => setSearch('')}><X size={16} /></button>}</div></div>
        <details className="collection-options"><summary>Filter & Sortierung{(filter!=='all'||tagFilter!=='Alle')&&<span> · Filter aktiv</span>}</summary><label className="sort-label"><span className="sr-only">Rezepte sortieren</span><select value={sort} onChange={e => setSort(e.target.value)}><option value="new">Zuletzt hinzugefügt</option><option value="az">Name A–Z</option><option value="time">Kürzeste Zubereitung</option></select></label><div className="filters" aria-label="Rezeptfilter">{[{ id: 'all', label: 'Alle Rezepte', icon: BookOpen }, { id: 'favorites', label: 'Lieblingsrezepte', icon: Heart }, { id: 'mine', label: 'Meine eigenen', icon: PencilIcon }, { id: 'quick', label: 'Bis 30 Minuten', icon: Clock }].map(f => <button className={filter === f.id ? 'active' : ''} aria-pressed={filter === f.id} key={f.id} onClick={() => setFilter(f.id)}><f.icon size={15} />{f.label}</button>)}</div>
        {data.recipes.some(r => r.tags.length) && <div className="tag-filter"><label>Schlagwort<select aria-label="Nach Schlagwort filtern" value={tagFilter} onChange={e => setTagFilter(e.target.value)}><option>Alle</option>{[...new Set(data.recipes.flatMap(r => r.tags))].sort().map(t => <option key={t}>{t}</option>)}</select></label></div>}</details>{filtered.length ? <div className="recipe-grid">{filtered.map(r => <RecipeCard key={r.id} recipe={r} onOpen={() => setDetailId(r.id)} onFavorite={() => favorite(r.id)} onPlan={() => startPlan(r)} />)}</div> : <div className="empty-state"><BookOpen size={40} strokeWidth={1.3} /><h3>{search ? 'Noch kein Treffer.' : filter === 'favorites' ? 'Platz für deine Lieblinge.' : 'Hier beginnt deine Sammlung.'}</h3><p>{search ? 'Probiere einen anderen Namen oder eine Zutat.' : filter === 'favorites' ? 'Tippe auf das Herz bei einem Rezept, um es hier zu sammeln.' : 'Fotografiere deine erste Rezeptseite oder füge einen Rezepttext ein.'}</p><button className="btn primary" onClick={() => search ? setSearch('') : setEditor({ tab: 'photo' })}>{search ? 'Suche zurücksetzen' : 'Rezept hinzufügen'}</button>{!search && <button className="text-btn" onClick={() => showView('discover')}>Oder neue Rezepte entdecken <ArrowRight size={15} /></button>}</div>}
      ...
      {view === 'shopping' && <>
        <div className="page-title"><div className="eyebrow">ALLES DABEI. KOPF FREI.</div><h1>Deine Einkaufsliste.</h1><p>Dein Wochenbedarf, abzüglich passender Vorräte.</p></div>
        ...
        <div className="stock-controls"><label className="checkbox-label"><input type="checkbox" checked={usePantry} onChange={e=>setUsePantry(e.target.checked)}/>Vorräte für diese Woche abziehen</label><p className="fineprint">Gleicher Lebensmittelname und passende Mengen werden verrechnet. Unklare Mengen bleiben auf der Liste. Verbrauch nach dem Kochen im Vorrat anpassen.</p><button className="text-btn" onClick={()=>{setKitchenTab('pantry');showView('kitchen');}}>Vorräte bearbeiten</button></div>
        <div className="shopping-layout"><section className="shopping-paper"><div className="shopping-progress"><div><ShoppingBasket size={20} /><strong>{items.length - checkedCount} {items.length - checkedCount === 1 ? 'Zutat fehlt' : 'Zutaten fehlen'} noch</strong><span>{checkedCount} von {items.length} erledigt</span></div><progress value={checkedCount} max={items.length || 1} /></div>
          {items.length ? [...new Set(items.map(i => i.category))].map(cat => <div className="shopping-category" key={cat}><h3>{cat}<span>{items.filter(i => i.category === cat).length}</span></h3>{items.filter(i => i.category === cat).map(i => <div className={`shopping-item ${isChecked(data, week, i) ? 'checked' : ''}`} key={i.key}><label><input type="checkbox" checked={isChecked(data, week, i)} disabled={coveredByPantry(i)} onChange={() => mutate(d => { const checked = { ...d.checked }; const key = `${week}|${i.key}`; if (isChecked(d, week, i)) delete checked[key]; else checked[key] = shoppingSignature(i); return { ...d, checked }; })} /><span className="custom-check"><Check size={14} /></span><span className="shopping-name"><strong>{i.name}</strong><small>{i.recipeTitles.length ? i.recipeTitles.join(' · ') : 'Selbst ergänzt'}</small>{!!i.pantryAmount&&<small className="stock-breakdown">Bedarf {ingredientText({...i,amount:i.requiredAmount!,name:''})} · Vorrat {ingredientText({...i,amount:i.pantryAmount,name:''})}</small>}</span><span className="shopping-amount">{coveredByPantry(i)?'Im Vorrat':i.amount === null ? i.extraId ? '' : 'nach Bedarf / prüfen' : ingredientText({ ...i, name: '' })}</span></label>{i.extraId && <button className="icon-btn" aria-label={`${i.name} entfernen`} onClick={() => mutate(d => ({ ...d, extras: d.extras.filter(e => e.id !== i.extraId) }))}><X size={16} /></button>}</div>)}</div>) : <div className="empty-state compact"><ShoppingBasket size={42} strokeWidth={1.2} /><h3>Deine Liste wartet auf dich.</h3><p>Plane ein Rezept ein oder ergänze unten, was du einkaufen möchtest.</p><button className="btn primary" onClick={() => showView('plan')}>Woche planen <ArrowRight size={16} /></button></div>}
        ...
        </section><aside className="shopping-aside"><div className="little-tip"><Leaf size={24} /><p><strong>Erst ein Blick in den Vorrat.</strong>Trage Mengen wie 500 g oder 3 Stück ein. Lebensmittel mit überschrittenem Verbrauchsdatum werden nicht abgezogen.</p></div>{plannedCount > 0 && <div className="list-origin"><CalendarDays size={21} /><strong>Für {plannedCount} geplante {plannedCount === 1 ? 'Mahlzeit' : 'Mahlzeiten'}</strong><button className="text-btn" onClick={() => showView('plan')}>Wochenplan ansehen <ArrowRight size={15} /></button></div>}{weekDates(week).some(day => data.plan[day]?.some(e => !data.recipes.find(r => r.id === e.recipeId)?.ingredients.length)) && <div className="notice">Bei mindestens einem geplanten Rezept fehlen noch Zutaten. Ergänze sie im Rezept, damit die Liste vollständig ist.</div>}</aside></div>
      </>}
    ...
    {settings && <Settings data={data} family={family} onConnect={()=>{setSettings(false);setKitchenTab('family');showView('kitchen');}} onReplace={commit} onClose={() => setSettings(false)} />}
    ...
    {needRefresh && <div className="update-banner" role="status"><span>Eine neue Version ist bereit.</span><button className="btn primary" onClick={() => { if (!editor && !aiPlanner && !mealPrep && !cookingBatch && !setupToken && !profileDirty) void updateServiceWorker(true); else setToast('Bitte speichere zuerst deine offenen Eingaben.'); }}>Aktualisieren</button><button className="icon-btn" aria-label="Update später" onClick={() => setNeedRefresh(false)}><X size={17} /></button></div>}
  </div>;
}
