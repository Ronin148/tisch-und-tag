...
const fallbackProfiles=defaultProfiles();
export default function Kitchen({ data,onChange,onAdd,onOpen,family,tab,setTab,onProfileDirty }: { data:AppData;onChange:(update:(d:AppData)=>AppData)=>Promise<void>;onAdd:(r:Recipe)=>Promise<void>;onOpen:(id:string)=>void;family:FamilySync;tab:string;setTab:(tab:string)=>void;onProfileDirty:(dirty:boolean)=>void }) {
  return <><div className="page-title"><h1>Unsere Küche</h1><p>Vorräte nutzen, saisonal kochen, gemeinsam planen.</p></div><div className="source-tabs kitchen-tabs">{[{id:'pantry',label:'Vorräte'},{id:'season',label:'Saison'},{id:'profiles',label:'Profile'},{id:'family',label:'Familie & KI'}].map(t=><button key={t.id} className={t.id===tab?'active':''} aria-pressed={t.id===tab} onClick={()=>setTab(t.id)}>{t.label}</button>)}</div>
    ...
    <div hidden={tab!=='profiles'}><Profiles profiles={data.profiles||fallbackProfiles} onDirtyChange={onProfileDirty} onSave={(profiles,original)=>onChange(d=>{
