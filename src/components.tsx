...
export function RecipeDetail({ recipe, onClose, onEdit, onPlan, onDelete, onFavorite, discovery = false, onImport, importBusy, alreadySaved, importError }: { discovery?: boolean; onImport?: () => void; importBusy?: boolean; alreadySaved?: boolean; importError?: string; recipe: Recipe; onClose: () => void; onEdit: () => void; onPlan: (portions: number) => void; onDelete: () => void; onFavorite: () => void }) {
  ...
  return <Modal title="Dein Rezept" wide onClose={onClose}>
    ...
    {recipe.source && <p className="fineprint">{recipe.source}</p>}{importError && <div className="notice error" role="alert">{importError}</div>}</div><div className="modal-footer">{discovery ? <button className="btn primary" onClick={onImport} disabled={importBusy}>{importBusy ? <LoaderCircle className="spin" size={18} /> : <Plus size={18} />}{alreadySaved ? 'Im Kochbuch öffnen' : 'Mit Bild ins Kochbuch übernehmen'}</button> : <button className="btn primary" onClick={() => onPlan(portions)}><CalendarPlus size={18} />In die Woche einplanen</button>}</div>
  </Modal>;
}
