...
export function RecipeEditor({ recipe, initialTab = 'photo', onClose, onSave }: { recipe?: Recipe; initialTab?: ImportTab; onClose: () => void; onSave: (r: Recipe) => Promise<void> }) {
  ...
  const importText = (text: string) => {
    ...
    if(link.trim()&&safeUrl(link.trim()))setDraft(d=>({...d,sourceUrl:link.trim()}));
    setError('');
  ...
  return <Modal title={recipe ? 'Rezept bearbeiten' : 'Ein neues Lieblingsrezept'} onClose={requestClose} wide>
    <form onSubmit={save}><div className="modal-body editor">
      ...
      {error && <div className="notice error" role="alert">{error}{tab==='link'&&!busy&&<><p>Du kannst das Rezept trotzdem übernehmen. Der Link bleibt als Quelle erhalten.</p><div className="button-row"><button type="button" className="btn" onClick={()=>{if(safeUrl(link.trim()))change({sourceUrl:link.trim()});setError('');setTab('text');}}>Rezepttext einfügen</button><button type="button" className="btn" onClick={()=>{if(safeUrl(link.trim()))change({sourceUrl:link.trim()});setError('');setTab('photo');}}>Foto verwenden</button></div></>}</div>}
